...
async fn list_themes_returns_available_themes() {
    ...
    for theme in themes {
        ...
        assert!(
            theme["color_scheme"].is_string(),
            "theme should have color_scheme"
        );
    }
}
...
async fn list_themes_contains_default_dark_and_high_contrast() {
    ...
    let themes = body["data"].as_array().expect("data should be an array");
    let theme_ids: Vec<&str> = themes.iter().filter_map(|t| t["id"].as_str()).collect();

    assert!(
        theme_ids.contains(&"default"),
        "Should contain default theme"
    );
    assert!(theme_ids.contains(&"dark"), "Should contain dark theme");
    assert!(
        theme_ids.contains(&"high-contrast"),
        "Should contain high-contrast theme"
    );
}
...
async fn homepage_includes_theme_css_variables() {
    ...
    assert!(
        body.contains("id=\"theme-variables\""),
        "Should have theme-variables style block"
    );

    // Should include CSS variables for themes
    assert!(
        body.contains("--theme-"),
        "Should include --theme- CSS variables"
    );
}
...
async fn homepage_includes_theme_switcher_script() {
    ...
    assert!(
        body.contains("theme-switcher.js"),
        "Should include theme-switcher.js"
    );
}
...
async fn account_settings_includes_theme_picker() {
    ...
    assert!(
        body.contains("Theme Preferences"),
        "Should include theme preferences section"
    );
    assert!(
        body.contains("themePicker"),
        "Should include theme picker element"
    );

    // Should include theme options
    assert!(
        body.contains("data-theme=\"auto\""),
        "Should include auto theme option"
    );
    assert!(
        body.contains("data-theme=\"default\""),
        "Should include light theme option"
    );
    assert!(
        body.contains("data-theme=\"dark\""),
        "Should include dark theme option"
    );
    assert!(
        body.contains("data-theme=\"high-contrast\""),
        "Should include high-contrast theme option"
    );
}
